...
import { popularBooksOverall, searchBooks, type BookSearchHit } from '../lib/openLibrary'
...
function ConcludePicker({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {shortlist.length === 0 ? (
        <p className="text-sm text-ink/70">Shortlist is empty. Add a rec, search below, or go back and nominate.</p>
      ) : (
        <ul className="flex flex-col gap-2">
          {shortlist.map((book) => (
            <li key={book.id}>

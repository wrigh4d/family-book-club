        ...
        >
            <header className="flex shrink-0 items-center justify-between gap-3 px-4 py-3 sm:px-6">
                ...
                <div className="flex shrink-0 gap-2">
                    <button type="button" className={presentActionClass()} onClick={() => navigate(`/club/${code}`)}>
                        Back to club
                    </button>
                    {owner && uid ? (
                        <button
            ...
            >
                <div className="flex h-full min-h-0 flex-col gap-3">
                    <CurrentBookStage current={current} facts={facts} ratingLabel={history ? ratingLabel : null}/>
            ...
            >
                {loop.map((book, index) => (
                    ...
                    >
                        ...
                        <p className="line-clamp-2 text-center font-display text-xs leading-tight">{book.title}</p>
                    </div>
                ))}
            </div>
        </section>
            ...
            <FadeDeck items={views} paused={paused}>
                {(view) => (
                    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
                        <p className="shrink-0 text-[11px] uppercase tracking-[0.2em] text-gold">{view.title}</p>
                    ...
                    return (
                        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
                            ...
                            <div className="mt-2 flex min-h-0 flex-1 gap-3 overflow-hidden">
                                <Cover src={slide.rec.coverUrl} title={slide.rec.title} className="h-24 w-[4.25rem]"/>
                                <div className="min-w-0 overflow-hidden">
                                    <h3 className="line-clamp-2 font-display text-lg leading-tight">{slide.rec.title}</h3>

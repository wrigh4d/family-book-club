...
const SEARCH_FIELDS =
  'key,title,author_name,cover_i,subject,first_publish_year,number_of_pages_median'
...
export async function popularBooksOverall(): Promise<BookSearchHit[]> {
  const url = `https://openlibrary.org/search.json?q=language:eng&sort=want_to_read&limit=8&fields=${SEARCH_FIELDS}`
